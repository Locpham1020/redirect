 ...
 * Version: 4.1.0 - FIX BỘ ĐỒNG BỘ GOOGLE SHEET - DORIK
 * Cập nhật: Sửa lỗi icon không click được, sửa lỗi không load dữ liệu
 */
      ...
      loadFromCache: function() {
        try {
          ...
          if (!parsedData._metadata) {
            if (ProductApp.config.debug) console.log('Cache không có metadata, xóa cache');
          ...
          if (ProductApp.config.debug) {
            console.log('Đã tải dữ liệu từ cache, thời gian:', new Date(cacheTime).toLocaleString());
            console.log('Số lượng container trong cache:', Object.keys(parsedData).filter(k => !k.startsWith('_')).length);
          }
          return true;
        } catch (e) {
          ...
          .then(text => {
            try {
              ...
              if (ProductApp.config.debug) {
                ...
                console.log(text.length > 2000 ? text.substring(0, 2000) + '...' : text);
              }
              ...
              const containers = Object.keys(data).filter(k => !k.startsWith('_'));
              if (containers.length === 0) {
                this.lastFetchStatus = 'empty';
              ...
              data._metadata.fetch_time = timestamp;
              ...
              this.lastFetchStatus = 'parse_error';
   ...
   init: function() {
     try {
       ...
       try {
         ...
         localStorage.removeItem('product_data_v4');
       } catch(e) {}
