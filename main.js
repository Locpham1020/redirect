...
(function() {
 ...
 const ProductApp = {
   ...
   config: {
     ...
     pollInterval: 60 * 1000, // Kiểm tra dữ liệu mới mỗi 1 phút
     version: '1.0.1',
     debug: true
   },
   ...
   uiManager: {
     ...
     updateAllContainers: function() {
       ...
       if (ProductApp.config.debug) console.log(`Đã cập nhật ${updatedCount}/${containerIds.length} containers`);
     }
   },
     ...
     observeContainers: function() {
       ...
       for (let i = 1; i <= 99; i++) {
         ...
         if (container) {
           ...
           if (ProductApp.config.debug) console.log(`Đang theo dõi container: ${containerId}`);
         }
       }
     }
   },
